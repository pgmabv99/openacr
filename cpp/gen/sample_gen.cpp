...
bool sample::rec_XrefMaybe(sample::FRec &row) {
    bool retval = true;
    (void)row;
    // insert rec into index ind_hashkey
    if (true) { // user-defined insert condition
        bool success = ind_hashkey_InsertMaybe(row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "sample.duplicate_key  xref:sample.FDb.ind_hashkey"; // check for duplicate key
            return false;
        }
    }
...
// --- sample.FDb.ind_hashkey.Find
// Find row by key. Return NULL if not found.
sample::FRec* sample::ind_hashkey_Find(i32 key) {
    u32 index = ::i32_Hash(0, key) & (_db.ind_hashkey_buckets_n - 1);
    sample::FRec* *e = &_db.ind_hashkey_buckets_elems[index];
    sample::FRec* ret=NULL;
    do {
        ret       = *e;
        bool done = !ret || (*ret).hashkey == key;
        if (done) break;
        e         = &ret->ind_hashkey_next;
    } while (true);
    return ret;
}

// --- sample.FDb.ind_hashkey.FindX
// Look up row by key and return reference. Throw exception if not found
sample::FRec& sample::ind_hashkey_FindX(i32 key) {
    sample::FRec* ret = ind_hashkey_Find(key);
    vrfy(ret, tempstr() << "sample.key_error  table:ind_hashkey  key:'"<<key<<"'  comment:'key not found'");
    return *ret;
}

// --- sample.FDb.ind_hashkey.InsertMaybe
// Insert row into hash table. Return true if row is reachable through the hash after the function completes.
bool sample::ind_hashkey_InsertMaybe(sample::FRec& row) {
    ind_hashkey_Reserve(1);
    bool retval = true; // if already in hash, InsertMaybe returns true
    if (LIKELY(row.ind_hashkey_next == (sample::FRec*)-1)) {// check if in hash already
        u32 index = ::i32_Hash(0, row.hashkey) & (_db.ind_hashkey_buckets_n - 1);
        sample::FRec* *prev = &_db.ind_hashkey_buckets_elems[index];
        do {
            sample::FRec* ret = *prev;
            if (!ret) { // exit condition 1: reached the end of the list
                break;
            }
            if ((*ret).hashkey == row.hashkey) { // exit condition 2: found matching key
                retval = false;
                break;
            }
            prev = &ret->ind_hashkey_next;
        } while (true);
        if (retval) {
            row.ind_hashkey_next = *prev;
            _db.ind_hashkey_n++;
            *prev = &row;
        }
    }
...
// --- sample.FDb.ind_hashkey.Remove
// Remove reference to element from hash index. If element is not in hash, do nothing
void sample::ind_hashkey_Remove(sample::FRec& row) {
    if (LIKELY(row.ind_hashkey_next != (sample::FRec*)-1)) {// check if in hash already
        u32 index = ::i32_Hash(0, row.hashkey) & (_db.ind_hashkey_buckets_n - 1);
        sample::FRec* *prev = &_db.ind_hashkey_buckets_elems[index]; // addr of pointer to current element
        while (sample::FRec *next = *prev) {                          // scan the collision chain for our element
            if (next == &row) {        // found it?
                *prev = next->ind_hashkey_next; // unlink (singly linked list)
                _db.ind_hashkey_n--;
                row.ind_hashkey_next = (sample::FRec*)-1;// not-in-hash
                break;
            }
            prev = &next->ind_hashkey_next;
        }
    }
}

// --- sample.FDb.ind_hashkey.Reserve
// Reserve enough room in the hash for N more elements. Return success code.
void sample::ind_hashkey_Reserve(int n) {
    u32 old_nbuckets = _db.ind_hashkey_buckets_n;
    u32 new_nelems   = _db.ind_hashkey_n + n;
    ...
    if (new_nelems > old_nbuckets) {
        ...
        if (UNLIKELY(!new_buckets)) {
            FatalErrorExit("sample.out_of_memory  field:sample.FDb.ind_hashkey");
        }
        memset(new_buckets, 0, new_size); // clear pointers
        // rehash all entries
        for (int i = 0; i < _db.ind_hashkey_buckets_n; i++) {
            sample::FRec* elem = _db.ind_hashkey_buckets_elems[i];
            while (elem) {
                sample::FRec &row        = *elem;
                sample::FRec* next       = row.ind_hashkey_next;
                u32 index          = ::i32_Hash(0, row.hashkey) & (new_nbuckets-1);
                row.ind_hashkey_next     = new_buckets[index];
                new_buckets[index] = &row;
                elem               = next;
            }
        }
        // free old array
        algo_lib::malloc_FreeMem(_db.ind_hashkey_buckets_elems, old_size);
        _db.ind_hashkey_buckets_elems = new_buckets;
        _db.ind_hashkey_buckets_n = new_nbuckets;
    }
}
...
void sample::FDb_Init() {
    ...
    _db.ind_hashkey_n             	= 0; // (sample.FDb.ind_hashkey)
    _db.ind_hashkey_buckets_n     	= 4; // (sample.FDb.ind_hashkey)
    _db.ind_hashkey_buckets_elems 	= (sample::FRec**)algo_lib::malloc_AllocMem(sizeof(sample::FRec*)*_db.ind_hashkey_buckets_n); // initial buckets (sample.FDb.ind_hashkey)
    if (!_db.ind_hashkey_buckets_elems) {
        FatalErrorExit("out of memory"); // (sample.FDb.ind_hashkey)
    }
    memset(_db.ind_hashkey_buckets_elems, 0, sizeof(sample::FRec*)*_db.ind_hashkey_buckets_n); // (sample.FDb.ind_hashkey)
...
void sample::FDb_Uninit() {
    ...
    // sample.FDb.ind_hashkey.Uninit (Thash)  //
    // skip destruction of ind_hashkey in global scope
}
...
void sample::FRec_Uninit(sample::FRec& rec) {
    ...
    ind_hashkey_Remove(row); // remove rec from index ind_hashkey
